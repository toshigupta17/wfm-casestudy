...
function PersonaGlyph({ persona }: { persona: PersonaGoal }) {
  ...
  return (
    <div className="relative mx-auto mb-5 flex h-16 w-16 items-center justify-center">
      ...
      <div
        className={`relative h-16 w-16 overflow-hidden rounded-full border-2 bg-slate-100 ${a.border}`}
      >
        <img
...
export function RequirementsPersonas() {
  ...
  return (
    <FadeIn>
      <div className="mb-12">
        <h3 className="mb-6 font-display text-xl font-semibold text-slate-900 md:text-2xl">{personasTitle}</h3>
            ...
            <li key={persona.role} className="h-full">
              ...
              >
                <GlassCard className="flex h-full flex-col p-6 md:p-7">
                  ...
                  <h4 className="mb-3 text-center font-display text-base font-semibold text-slate-900">
                    {persona.role}
                  </h4>
                  <p className="text-center text-[0.95rem] leading-relaxed text-slate-600">{persona.goal}</p>
                </GlassCard>
              </motion.div>
            </li>
          ))}
